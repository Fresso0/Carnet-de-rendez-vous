...
RDV::RDV(const string &nom,const int &j,const int& m, const int& a,const int& hd,const int&hf):
    d_nom{nom},d_jour{j},d_mois{m},d_an{a},d_hDebut{hd},d_hFin{hf}{}
...
//GETTERS
string RDV::retourneNom() const
{
    return d_nom;
}
int RDV::retourneHeureDebut() const
{
    return d_hDebut;
}
int RDV::retourneHeureFin() const
{
    return d_hFin;
}
int RDV::retourneJour() const
{
    return d_jour;
}

int RDV::retourneMois() const
{
    return d_mois;
}

int RDV::retourneAn() const
{
    return d_an;
}
...
string RDV::afficherListeParticipants() const
{
    ...
    if(d_listeParticipants.size()==0)
    {
        ...
        for (int i = 0; i < static_cast<int>(d_listeParticipants.size()); i++)
        {
            ostringstream oss;
...
void RDV::retirerParticipant(participant &p)  //supr le rdv this du participant p
{
    ...
    while(i<static_cast<int>(d_listeParticipants.size()) && (d_listeParticipants[i].retourneNom()!=p.retourneNom()||d_listeParticipants[i].retournePrenom()!=p.retournePrenom()))
    {
        i++;
    }
...
vector<participant> RDV::getParticipants() const {
    return d_listeParticipants;
}
